...
The MainActivity is the activity that starts on application startup. It uses the main.xml as its layout file, which is a layout file containing three buttons, which can be seen on figure \textcolor{red}{INDSÆT SCREEN CAP AF MAINACTIVITY}. A snap of the MainActivity's code can be seen in listing \vref{lst:mainactivity}.
...
The FragParentTab is the activity, which, as stated above, is started by the MainActivity activity. The activity has the responsibillity of managing what view to show, by using fragments. The reason for choosing fragments instead of making new activities is that the layout we want is a tab layout. The layout can be seen in figure \textcolor{red}{Indsæt FIGUR REF}. The FragParentTab activity can be seen in listing \vref{lst:fragparenttag}.
