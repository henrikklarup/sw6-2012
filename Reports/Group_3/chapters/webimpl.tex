...
\begin{description}
	\item[\code{<input type=Text/Password/TextField/>}] The value of these fields are of the type string, and contains the value entered into the text field on the page.
	\item[\code{<input type=RadioButton value=``some_value''/>}] All \code{RadioButton}s with the same name, sends \code{some_value} of the selected \code{RadioButton}.
	\item[\code{<input type=CheckBox value=``some_value''/>}] All \code{CheckBox}es with the same name sends \code{some_value} of each checked box.
	\item[\code{<select ...><option value=``some_value''>caption</ ... >}] Sends \code{some_value} of the selected item.
	\item[\code{<select ... multiple=``multiple''><option value=``some_value''>caption</ ...>}] Sends \code{some_value} of all the selected items as a string array.
\end{description}
