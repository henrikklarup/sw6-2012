...
The \activity{ProfileSelectActivity} activity implements the behavior described in \autoref{design:profile_selection}, and takes care of providing the profile service, described in \autoref{sec:launcher_architecture} \todo{Fix autoref}, to the launched \girafapp[].

As explained in \autoref{sec:launcher_architecture} \todo{Fix autoref}, the \guardian[] should be able to launch apps with all profiles which are attached to the \guardian[]. \autoref{lst:snipgetprofiles} shows how all profiles are found, including those in attached departments, and their subdepartments.
