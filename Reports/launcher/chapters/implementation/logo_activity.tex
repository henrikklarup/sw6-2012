...
The logo activity implements TIME-AUTO-LOGIN-FEATURE -- explained in \autoref{sec:autologin} -- by using the \method{sessionExpired} method, found in \class{Tools} class, seen in \autoref{lst:sessionexpired}. The usage of the \method{sessionExpired} method can be seen in \autoref{lst:sessionexpiredusage}
...
\begin{lstlisting}[style=sourceCode, language=JAVA, caption=LogoActivity.java, label=lst:sessionexpired]
