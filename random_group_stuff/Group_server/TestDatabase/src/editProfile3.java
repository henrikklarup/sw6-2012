 ...
 */
public class editProfile3 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else
		{
			...
			if (changedMade != null && changedMade.equals("2"))
			{
				...
				session.setAttribute("MESSAGETOUSER", "Intet ændret. Statuskode = 0");
			}
			...
			if ((changedMade != null) && (changedMade.equals("1")))
			{
				...
				if (oldPass != null && !oldPass.equals("") && !oldPass.equals(password))
				{
					session.setAttribute("MESSAGETOUSER", "Intet ændret - forkert kodeord. Statuskode = 201");
					response.sendRedirect("main");
					return;
				}
				else if ((newPass != null || newPass1 != null) && !newPass.equals(newPass1))
				{
					session.setAttribute("MESSAGETOUSER", "Intet ændret - De 2 kodeord er ikke ens. Statuskode = 202");
					response.sendRedirect("main");
					return;
				}
				else if ((newPass != null) && (newPass1 != null) && (oldPass != null) && newPass.equals(newPass1) && oldPass.equals(password))
				{
				...
				try {
					...
					int test = stmt.executeUpdate("UPDATE Profile SET" +
							...
							" surname='"+surname+"',"+
							" middlename='"+middlename+"'"+ updatePassword +
							" WHERE idProfile="+userId+";");
					session.setAttribute("MESSAGETOUSER", "Profil ændret. Statuskode = " + test);
					response.sendRedirect("main");
				}
				catch (Exception e) {
			...
			if (pRole == 0)
			{
				dropDownMenu = dropDownMenu + "<option selected='yes'>Admin</option>" + 
			...
			out.println("</div>");
			out.println("<div id=\"profileSite3\">");
			...
			out.println("<tr>");
			out.println("<td>Navn " + user +":<td><input type=text name='firstname' value='"+firstname+"'>");
			out.println("</tr>");
			out.println("<tr>");
			out.println("<td>Mellemnavn:<td><input type=text name='middlename' value='"+middlename+"'>");
			out.println("</tr>");
			out.println("<tr>");
			out.println("<td>Efternavn:<td><input type=text name='surname' value='"+surname+"'>");
			out.println("</tr>");
			out.println("<tr>");
			out.println("<td>Rolle:<td>" + dropDownMenu);
			out.println("</tr>");
			out.println("<tr>");
			out.println("<td>Telefon nummer:<td><input type=text name='phone' value='"+phone+"'>");
			out.println("</tr>");
			out.println("<tr>");
			out.println("<td>Brugernavn:<td><input type=text name='username' value='"+username+"' disabled='true'>");
			out.println("</tr>");
			out.println("<tr>");
